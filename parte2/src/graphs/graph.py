# graph.py (Refatorado para ser um módulo importável)
...
def build_game_graph(csv_path):
    
    nodes_data = {}
    edge_list_with_genre = [] 
    ...
    print(f"Arquivo '{csv_path}' lido com sucesso. Total de {len(df)} registros.")
    ...
    for i, (genre, nodes_in_genre) in enumerate(nodes_by_genre.items()):
        ...
        print(f"  [Gênero {i+1}/{total_genres}] Processando: '{genre}' ({num_nodes_in_genre} nós)")
        ...
        for node1, node2 in pares_no_genero:
            ...
            edge_list_with_genre.append((node1, node2, peso_aresta, genre)) 
    ...
    return nodes_data, edge_list_with_genre

if __name__ == '__main__':
    
    print("Executando 'graph.py' como script principal...")
    ...
    if not edge_list:
        print("Nenhuma aresta foi criada.")
    else:
        for i, (n1, n2, peso, genero) in enumerate(edge_list):
            if i >= 10:
                break
            print(f"Aresta: ({n1}, {n2}) | Peso: {peso:.2f} | Gênero: {genero}")